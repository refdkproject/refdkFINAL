...
export default function AdminWishlistDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/admin/wishlist/${eventId}`, {
          method: "GET",
          headers: {
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] bg-[#E5EAE7] p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-[#2D5A4C] mb-6">Discover Wishlist</h1>
        ...
        <div className="grid gap-8">
          <h2 className="text-3xl font-bold text-[#2D5A4C]">WISHLIST DETAILS</h2>
            ...
            {loading ? (
              ...
              <Card key={event._id} className="bg-white min-w-[50%]">
                <CardContent className="p-6">
                  {event.eventPic && (
                    <img
                      src={`${import.meta.env.VITE_BASE_URL}/${event.eventPic.replace(/\\/g, "/")}`}
                      alt={event.name}
                      className="w-[60%] h-auto m-auto rounded-lg"
                    />
                  )}
