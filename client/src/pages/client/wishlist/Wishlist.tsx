...
export default function WishListPage() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/wishlist`, {
          method: 'GET',
          headers: {
  ...
  return (
    <main className='min-h-[calc(100vh-4rem)] bg-[#E5EAE7] p-8'>
      <div className='max-w-6xl mx-auto'>
        <h1 className='text-4xl font-bold text-[#2D5A4C] mb-6'>Discover WishLists</h1>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setEvents((prevEvents) =>
                prevEvents.filter((event: any) => event.title.toLowerCase().includes(searchTerm))
              );
            }}
          />
        ...
        <div className='grid gap-8'>
          <div>
            <h2 className='text-3xl font-bold text-[#2D5A4C] mb-8'>Community Wishlist</h2>
            <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-2'>
              {events.map((event: any) => (
                <Card key={event._id} className='bg-white min-w-[50%]'>
                  <CardContent className='p-4'>
                    <img className='w-full h-[200px] object-cover' src={event.mediaUrl} alt={event.title} />

                    <h3 className='font-semibold mt-2'>Name: {event.title}</h3>
                    <h4 className='text-sm my-3'>Description: {event.tags}</h4>
                    <p>Wishlist: </p>
